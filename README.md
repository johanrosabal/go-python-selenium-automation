...
### 1. Datos Individuales (JSON)
Cada test carga sus datos de un JSON nombrado como el ID del test:
```json
{
  "tests": {
    "id": "CT-LOGIN-001",
...
### 2. Uso del Decorador `@test_case` (Recomendado)
Con el nuevo patrón **App Orchestrator**, no necesitas importar Page Objects. Simplemente usa `self.app`:

```python
@test_case(id="CT-LOGIN-001")
def test_valid_login(self):
    data = self.get_data_for_test()
    # Acceso centralizado y Lazy Loading
